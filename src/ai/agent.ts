...
export async function handleUserMessage(input: string): Promise<{message: string, patch: any[]}> {
  ...
  if ((/фон|background/.test(lower)) && (hex || url)) {
    ...
    if (!patch.length) {
      return { 
        message: `Не нашёл подходящее место для фона в слое ${layer}. Скажи точнее (например: "фон home", "фон lock").`, 
        patch: [] 
      };
    }
    
    return { 
      message: `Обновил фон слоя ${layer}${url ? ' (картинка)' : ` на ${hex}`}`, 
      patch 
    };
  }
  ...
  if ((/button|кнопк/.test(lower)) && hex) {
    ...
    if (!patch.length) {
      return { 
        message: `Не нашёл подходящую кнопку в слое ${layer}. Скажи точнее (например: "кнопка swap", "кнопка send").`, 
        patch: [] 
      };
    }
    
    return { 
      message: `Сделал кнопку слоя ${layer} → ${hex}`, 
      patch 
    };
  }
  ...
  if ((/текст|text/.test(lower)) && hex) {
    ...
    if (!patch.length) {
      return { 
        message: `Не нашёл подходящий текст в слое ${layer}. Скажи точнее (например: "текст home", "текст заголовка").`, 
        patch: [] 
      };
    }
    
    return { 
      message: `Текст на слое ${layer} теперь ${hex}`, 
      patch 
    };
  }
  ...
  return { 
    message: `Что изменить? Примеры команд:
