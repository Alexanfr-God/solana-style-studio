...
export const LoginScreenPreview = ({ style }: { style: WalletStyle }) => {
  ...
  // Use the actual loginStyle from walletCustomizationStore instead of passed style
  ...
  const getPhantomLabelColor = () => {
    const phantomColor = currentStyle.textColor || '#FFFFFF';
    
    // Check if we need to adjust contrast
    if (currentStyle.backgroundColor) {
      return getTextContrast(currentStyle.backgroundColor);
    }
