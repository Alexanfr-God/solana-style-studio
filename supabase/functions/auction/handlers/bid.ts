...
export async function handlePlaceBid(
  ...
  try {
    ...
    if (STUB_MODE) {
      console.log('[place-bid] 🧪 STUB_MODE: skipping on-chain verification');
      if (!finalTxSignature) {
        finalTxSignature = `stub_bid_tx_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
      }
    } else {
    ...
    const bid = await createBid({ auction_id, bidder_wallet, bid_price_lamports, tx_signature: finalTxSignature });
    ...
    return {
      success: true,
      data: { bid, new_current_price: bid_price_lamports },
      ...(STUB_MODE && { message: 'STUB MODE: No real SOL transfer verified' }),
    };
  } catch (error) {
